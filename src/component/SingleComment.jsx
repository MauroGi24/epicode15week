...
function SingleComment({ comments, loadComments }) {
  const deleteComment = async () => {
    try {
      const response = await (fetch(`https://striveschool-api.herokuapp.com/api/comments/${comments._id}`, {
      headers: {
  ...
  return (
    <>
      ...
      <Button type="button" onClick={deleteComment} className="btn btn-danger mb-2 me-3">
        <svg
